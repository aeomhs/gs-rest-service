...
`name` 매개변수는 기본적으로 설정되어있는 `World`를 대체합니다. 그리고 다음과 같이 응답에 반영됩니다.
...
`@GetMapping` 어노테이션은 `/greeting`에 대한 HTTP GET 요청을 `greeting()` 메서드에 사상하는 것을 뜻합니다.

NOTE: `GET`이외의 HTTP 메소드를 위한 어노테이션도 존재합니다. (e.g. `@PostMapping` for POST)
